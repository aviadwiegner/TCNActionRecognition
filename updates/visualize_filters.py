...
def to_joint_dict(feature):
  ...
  body1 = feature[:int(feat_dim/2)]
  body2 = feature[int(feat_dim/2):]
  for index in range(0,int(feat_dim/2),step):
...
def vis_filters():
  model = Models.TCN_resnet(n_classes,
                            feat_dim,
                            max_len,
                            gap=1,
                            dropout=0.0,
  ...
  conv1_weights = conv1.get_weights()[0]

  num_filters = conv1_weights.shape[-1]


  for filter_ind in range(0,num_filters):
   filter1 = conv1_weights[:,:,filter_ind]
   plt.imshow(np.transpose(filter1),cmap="jet")

   outdir = "./filter_vis/tk_tcn_xyz_d50_c25_128x2_256x2_512x2/"
   if not os.path.exists(outdir):
     os.makedirs(outdir)

   plt.savefig(outdir+"conv1_"+str(filter_ind)+"png",bbox_inches='tight')


   # top_values = sorted(filter1.flatten())[-3:]
   # temporal_max = []
   # response_max  = []
   # for val in top_values:
   #   max_time, max_dim =  np.where(filter1 == val)
...
def activate_joint_by_filter():
  ...
  conv1_filters = model.layers[1].get_weights()[0]
  conv1_activation = get_activations(model,layer_ind, X)[0]
  ...
  for f in range(0,conv1_filters.shape[-1]):
    ...
    for t in range(0,a_conv1_filter.shape[0]):
      new_joint_pos = {}
      for jnum, pos in initial_config.items():
